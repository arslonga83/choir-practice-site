import"./main-b0e50acb.js";import{i as m}from"./customPlayers-b2c5c59d.js";function p(a){let s=new Date(Date.parse(a));const o={weekday:"long",month:"long",day:"numeric"};return s.toLocaleString("en-US",o)}f();async function f(){const s=await(await fetch("recordings")).json(),o=document.getElementById("mixedRecordings");let t="";for(let e of s){let l="",n="",r="",c="",i="";if(new Date(Date.parse(e.date))<=Date.now()-3600*1e3*24)continue;let d=p(e.date);e["anthem-type"]=="youtube"?l=`
        ...
        `),t+=` 
          <div class="card text-center col-lg-5 col-md-12 rounded bg-primary text-white mt-5" >
            ...
            ${e.anthem?`
            <p class="card-subtitle py-2 fs-5 fw-bold">${e.anthem}</p> ${l}`:'<h3 class="py-3">Anthem TBD</h1>'}
            ${e.offertory?` 
            <p class="card-subtitle py-3 fs-5 fw-bold">${e.offertory}</p>
            ${n}`:'<h3 class="py-3">Offertory TBD</h1>'}
            ${e.song3?`${r}`:""}
            ${e.song4?`${c}`:""}
            ${e.song5?`${i}`:""}
            </div>
          </div>`}o.innerHTML+=t,m()}u();async function u(){const s=await(await fetch("schedule")).json(),o=document.getElementById("schedule-list-group");let t="",e=0;for(let l of s)new Date(Date.parse(l.date))>=Date.now()-3600*1e3*24&&e<=9&&(t+=`
      <li class="list-group-item d-lg-flex  align-items-center justify-content-center text-center">
        <p class="mb-lg-0 mb-md-1">${l.date}</p>
        <h5 class="mx-auto mb-lg-0">${l.name}</h5>
        <p class="mb-lg-0 mb-md-1">${l.time}</p>
